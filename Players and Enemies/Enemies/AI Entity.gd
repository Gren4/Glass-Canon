...
########################
#export(NodePath) var Enemy_path1
#onready var Enemy1 = get_node(Enemy_path1)
#export(NodePath) var Enemy_path2
#onready var Enemy2 = get_node(Enemy_path2)
...
func move_to(target):
	...
	if dist_to_player.length() >= 10 and col_enem > 2:
		...
		if dist_to_pl < 1.0:
			path = nav.get_nav_link_path(target.global_transform.origin, plV3)#["complete_path"]
			if path.has("complete_path"):
				target.get_nav_path(path["complete_path"])
		else:
			path = nav.get_nav_link_path(target.global_transform.origin, closest_p)#["complete_path"]
			if path.has("complete_path"):
				target.get_nav_path(path["complete_path"])
	else:
		...
		if dist_to_pl < 1.0:
			path = nav.get_nav_link_path(target.global_transform.origin, player.global_transform.origin)#["complete_path"]
			if path.has("complete_path"):
				target.get_nav_path(path["complete_path"])
		else:
			path = nav.get_nav_link_path(target.global_transform.origin, closest_p)#["complete_path"]
